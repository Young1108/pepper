    ...
    try:
        ...
        while True:
            ...
            if frame and frame[6]:
                ...
                array = np.frombuffer(frame[6], dtype=np.uint16)  # 每个深度值是2字节
                image = array.reshape((height, width))  # 对于2字节的深度值，不需要第三维

                # 转换为可视化的灰度图像
                image_visual = cv2.convertScaleAbs(image, alpha=0.08)  # alpha调节以获得最佳视觉效果
                ...
                if current_time - start_time > 0:
                    ...
                    cv2.putText(image_visual, "FPS: {:.2f}".format(fps), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)

                # 显示深度图像
                cv2.imshow("Pepper Depth Camera", image_visual)
...
if __name__ == "__main__":
    ...
    # 订阅摄像头参数
    name = "depCam2"  # name can only use 6 times
    camera_id = 3
    resolution = 15
    ...
    color_space = 11  # 11:kRGBColorSpace 13:kBGRColorSpace
